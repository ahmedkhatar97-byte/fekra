...
# 2. الستايل (نسف البياض السفلي + نيون + سبلاتش سكرين)
st.markdown("""
<style>
    footer {visibility: hidden;}
    header {visibility: hidden;}
    
    /* توحيد الخلفية الداكنة */
    .stApp, [data-testid="stAppViewContainer"], [data-testid="stBottom"] {
        background-color: #0E1117 !important;
    }

    /* معالجة حاوية الكتابة لمنع البياض */
...
if prompt := st.chat_input("بماذا تفكر يا حريف؟"):
    ...
    with st.chat_message("assistant"):
        try:
            ...
            full_history = [{"role": "system", "content": f"أنت Fekra AI، صممك أحمد وائل الحريف. الوقت: {current_time_info}."}] + st.session_state.messages
            
            res = client.chat.completions.create(model="llama-3.3-70b-versatile", messages=full_history, stream=True)
